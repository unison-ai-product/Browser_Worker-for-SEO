...
## 1.5. 自社記事が上位にいるとき（通しで唯一の確認）

① の `own_in_serp` に 10 位以内の自社記事があれば、② に入る前に AskUserQuestion で 1 回だけ聞く: 「既存記事 <URL>（<順位> 位）のリライト案として進める（推奨）/ 別記事として新規に作る / ここで止める」。新規記事を最後まで作ってから敵対検証で重複が分かると、③④ が丸ごとやり直しになる（2026-09-17 実機で発生）。リライト案で進める場合、② の上位 5 記事に自社記事を必ず含め、③ は「既存の見出し構成に対する差分（追加・統合・削除）」として構成案を作り、④ は差分の本文を書く。ゲートには、既存の見出しに差分を差し込んだ完成形（`article_merged_for_gate.md`）をかける（差分だけをかけると構造判定が誤って FAIL する）。
