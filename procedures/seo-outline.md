...
4. **ユーザーオリジナルメモリ**: `knowledge/memory/original.md`（主張・一次情報・事例・数値）と `knowledge/feedback/lessons.md` を読み、差別化要素（② 手順9）と突き合わせて**自社だけが書ける節**を最低 1 つ入れる。無ければ**聞かずに**立場 neutral で進め、`outline_notes.md` と handover.md に「一次情報が無い。自社だけが書ける節は未確定」と書く（通しは止まらない。単体 /構成案 のときだけ、完了報告の最後に「入れたい主張・事例があれば /SEO設定 memory で登録できます」と 1 行添える）。
5. **タイトルと見出しの作成**: skills/seo-outline の型（タイトル 32 字・H2 3〜6・H3 各 1〜3・FAQ 3〜5・まとめ）で第1案を書く。
6. **site:検索で内部リンク候補**（`own_domain` が空なら省略し、全 H2 を「内部リンク候補なし（未設定）」とする）: H2 ごとに、その見出しに含まれる SEO キーワードで Google の `site:` 検索をする。**検索窓に打ち込まず、browser_batch の 1 回にまとめる**: H2 の数だけ `navigate https://www.google.com/search?q=site:<own_domain>+<H2 のキーワード>&hl=ja&gl=jp&pws=0` → javascript_tool ← `templates/js/serp-extract.js` を並べ、返った `organic`（タイトル・URL）から各 H2 に 0〜2 本の自社記事を割り当てる。無い H2 は「内部リンク候補なし（新規記事候補）」と記録。`captcha: true` が返ったら残りの検索をやめ、取れた分だけで進める（突破しない）。
