...
新しい版が出ると、セッションの最初に「プラグインの新しい版 vX.Y.Z があります」と 1 行出ます。Cowork の 設定 → プラグイン で更新してください（更新後は `/SEO検証` で点検）。この確認は配布リポジトリの公開ファイルを 1 日 1 回読むだけで、送信する情報はありません。止めるには `knowledge/config/packs.conf` に `update_check=off`。
