...
## やること（手順書の 1〜7）
1. 渡された `memory/work/<kw>/pages/<順位>.json`（見出し階層・meta・H2 配下の内部リンク・JSON-LD・日付・著者・文字数。① が取得済み）を Read。本文の主張を読むために WebFetch を 1 回。**ブラウザは使わない**（5 体が同時に動くので 1 つのブラウザを取り合うと直列になる）。WebFetch で本文が取れなければ「本文未取得」と返し、構造だけで 2・3・6 を埋める。
2. タイトル / H1 / H2〜H4 階層 / meta description / 日付 / 著者 / 文字数は JSON の値を写す。
3. H2 ごとの内部リンク（JSON の `sections`）: 見出し → リンク先タイトル・URL。外部リンクは件数。ナビゲーション・CTA ボタンは除く。
