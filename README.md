...
| コマンド | やること |
|---|---|
| `/SEO記事 <キーワード> [構成案まで]` | ①〜④を通しで。構成案と WP 下書き投稿の前にあなたの承認 |
| `/SERPs解析` `/記事分析` `/構成案` `/記事作成` | 各段階の単体実行 |
| `/SEO設定` | 設定・記憶・機能 ON/OFF |
| `/SEO検証` | セルフテストと状態確認 |

普通に「〇〇で記事作って」と頼めば `/SEO記事` に振り分けます。
...
スキルの中身は次の資料から項目を転記して作り、サイト固有の語は `templates/site-profile.example.yaml` に分離しています。
初心者向け設計フォーマット.xlsx / SEO内部指示.xlsx / SEOライティング 基礎マニュアル（2023-12-04）/ AI-office-de-seo 要件定義 v3.7 / takumi-cmo search-console-jp・ga4-jp。
