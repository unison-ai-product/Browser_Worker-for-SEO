...
2. facts の「要修正（根拠）」を根拠どおりに直す。「要出典」は出典が facts に無ければ**その文を弱める（断定→傾向）か削る**。「要人間判断」の文は、**本文では断定を外した安全側の書き方にして残す**（AI が可否を決めて言い切らない。プレースホルダも残さない）。そのうえで `handover.md` に「該当箇所（見出し名・冒頭 20 字）/ 何を決める必要があるか / 選択肢 / 根拠」を 1 件ずつ書く。**要人間判断が残っていても止めない** — ゴールは WP 下書きで、決めるのは公開前の人間（下書きと一緒に handover.md を渡す）。
...
## 返答
保存パス 3 つ、文字数、handover.md に回した要人間判断の件数と箇所、削った文の一覧（理由付き）。10 行以内。
