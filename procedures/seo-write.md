...
2. **執筆**: unit-drafter（Sonnet, effort medium）を **U1 → U2 → U3 の順に直列**で起動（前ユニットの本文を次に渡し、文体・用語を揃える）。各体に brief・media-rules・skills/seo-writing の絶対パスを渡す。出力は `units/U<n>.draft.md`。
3. **図解**: 手順2 と**並列**に diagram-maker（Sonnet）を H2 テーマごと（最大 4 体同時）に起動し、`skills/diagram-maker` の型（SVG または Mermaid + alt テキスト + キャプション）で `memory/work/<kw>/figures/<h2-slug>.svg` を作らせる。
...
6. **ルール＆レギュレーションゲート**（通るまで修正ループ、最大3周）:
   - 機械判定: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/keyword-gate.py --article memory/work/<kw>/article.md --required memory/work/<kw>/required_keywords.txt --rules knowledge/rules/gate_rules.yaml`（必須KW網羅・禁止語・文字数・見出し数・リンク数・alt 欠落）。
