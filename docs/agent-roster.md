...
|---|---|---|---|---|
| serp-collector | 実行 / Sonnet | low | Google SERP を読み取り、AIO・順位・PAA・関連語を定義どおりに抽出 | ① |
| article-analyzer | 実行 / Sonnet | low | 上位記事 1 本の見出し・内部リンク・主張分類・AIO 引用箇所・構造化データを抽出 | ②（記事ごとに 1 体） |
| outline-builder | 実行 / Sonnet | medium | 分析の統合（②）・構成案の組み立てと修正（③）・ユニット brief と style.md（④）。メインが自分で組み立てないための係 | ②③④ |
| unit-drafter | 実行 / Sonnet | medium | ユニット 1 つの本文をライティングスキルと媒体ルールで執筆 | ④（U1→U2→U3 順次。fact-checker と重ねて流す） |
| diagram-maker | 実行 / Sonnet | low | H2 テーマの図解（SVG / Mermaid + alt + キャプション）を生成 | ④（並列 ≤4） |
